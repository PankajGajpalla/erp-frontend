...
function StudentFees({ studentId }) {
  ...
  if (loading) return <p className="text-gray-500">Loading fees...</p>
    ...
    <div className="space-y-6">
      ...
      {pendingFees.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-red-700">You have {pendingFees.length} pending payment{pendingFees.length > 1 ? "s" : ""}</p>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        ...
        {displayFees.length === 0 ? (
          <p className="p-6 text-gray-400">No records found.</p>
        ) : (
              ...
              {displayFees.map((f) => {
                ...
                return (
                  <tr key={f.id} className={`border-t transition
                    ...
                    <td className="px-6 py-3 text-green-600 font-medium">₹{f.paid}</td>
                    <td className="px-6 py-3 text-red-600 font-medium">₹{pending.toFixed(2)}</td>
