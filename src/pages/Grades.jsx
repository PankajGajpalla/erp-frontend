...
function AddGrades() {
  ...
  const notDoneCount  = students.length - absentCount - skippedCount
          ...
          <div className="p-5 border-b bg-blue-50">
            <div className="flex flex-wrap gap-x-6 gap-y-1 items-center">
              ...
              <div className="ml-auto text-right">
                <p className="text-xs text-gray-500">{filledCount} / {notDoneCount} filled</p>
            ...
            <tbody>
              {students.map((s) => {
                ...
                const letterGrade = pct !== null
                  ...
                  : null
                return (
                  <tr key={s.id} className={`border-t transition ${isAbsent ? "bg-orange-50/60 opacity-70" : isSkipped ? "bg-gray-50/80 opacity-70" : isOver || isNeg ? "bg-red-50" : "hover:bg-gray-50"}`}>
                    <td className="px-5 py-3">
                      <p className={`font-medium ${status ? "text-gray-400 line-through" : "text-gray-800"}`}>{s.name}</p>
